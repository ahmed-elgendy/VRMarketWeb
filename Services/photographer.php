...
$c=0;	//echo $upds['0']["img_id"];
	for($i=0;$i<count($upds);){
			for($j=0;$j<2;$j+=1)
				{
      if($c<=(count($upds)-1)){echo '<div class="col-lg-4 fadeInUp wow bounceInRight data-wow-delay="'.abs($x).'s" style="visibility: visible; animation-delay: '.abs($x).'s; animation-name: bounceInUp;">';
        echo '<div class="G">';
        
        $id = $upds[$c]['photographer_id'];


        echo '<a href="ms.php?imgid=' .$upds[$c]['photographer_id'] . '">';
        echo '<div class="item">';
        echo "<img src='" .$upds[$c]['photographer_img'] .  "'/></a><br>";
        echo '</a>';
        
echo '<div class="over text-center">';
        echo '<h4>Name:<small>'. $upds[$c]['photographer_name'].' </small></h4>' ;
      //  echo '<br>';
        
        echo '<h5>Description:<small>' .$upds[$c]['photographer_description'].'</small> </h5>' ;
       // echo '<br>';
       echo '</div>';
       echo '</div>';
